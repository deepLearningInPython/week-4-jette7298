...
punct = ".,;:?!()[]{}\"'/" 
tokens = [word for word in "".join([ch for ch in text if ch not in punct]).split()]
...
#define frequenciy functions

word_frequencies = {word: tokens.count(word)
...
def make_vocabulary_map(documents: list) -> tuple:
    ...
    # build unique tokens in first-occurrence order
    unique_tokens = []
    for w in all_tokens:
